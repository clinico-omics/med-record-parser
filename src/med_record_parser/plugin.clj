(ns med-record-parser.plugin
  "Simple plugin loader for med-record-parser.
  Medical Record Extractor. already offers the ability to load jar files in the classpath
...
(defn load-plugins
  ...
  ([options]
   ...
   (let [plugin-desc? (partial re-matches #"mre_plugin/(.*)/meta.edn$")
...
(defn load-plugin
  ...
  ([plugin]
   (load-from-resource (format "mre_plugin/%s/meta.edn" (name plugin))
                       {:alias true}))
  ([plugin options]
   (load-from-resource (format "mre_plugin/%s/meta.edn" (name plugin))
                       options)))